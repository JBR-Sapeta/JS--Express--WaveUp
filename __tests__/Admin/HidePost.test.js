...
const putHidePost = (postId, options = {}) => {
  const agent = request(app).put(`/api/v1.0/admin/posts/hide/${postId}`);
...
describe('PUT /api/v1.0/admin/posts/hide/:postId', () => {
  describe('Valid Request', () => {
    it('returns 200 code when request is valid', async () => {
      ...
      const response = await putHidePost(post.id, { token });
    ...
    it('returns success mesage when request is valid', async () => {
      ...
      const token = await getToken();
      const response = await putHidePost(post.id, { token });
      expect(response.body.message).toBe(en.post_hiden);
    });

    it('sets isPublic property to false in database', async () => {
      const user = await addUser();
      const post = await addPost(user.id);
      await addAdmin();
      const token = await getToken();
      await putHidePost(post.id, { token });
      const [postsInDB] = await PostModel.findAll();
      expect(postsInDB.isPublic).toBe(false);
    });
  });
  ...
  describe('Inalid Request', () => {
    it('returns 401 code when request is send without token', async () => {
      const user = await addUser();
      const post = await addPost(user.id);
      const response = await putHidePost(post.id);
    ...
    it('returns 401 code when request is send with invalid token', async () => {
      const user = await addUser();
      const post = await addPost(user.id);
      const response = await putHidePost(post.id, { token: 'Invalid_Token' });
      expect(response.status).toBe(401);
    });
    ...
    it('returns 404 code when request is send for post that does not exist', async () => {
      await addAdmin();
      const token = await getToken();
      const response = await putHidePost('does_not_exist', { token });
  ...
  describe('Internationalization', () => {
    ...
    `(
      ...
      async ({ language, message }) => {
        ...
        await addAdmin();
        const token = await getToken();
        const response = await putHidePost(post.id, { token, language });
    ...
    `(
      ...
      async ({ language, message }) => {
        const user = await addUser();
        const post = await addPost(user.id);
        const response = await putHidePost(post.id, { language });
        expect(response.body.message).toBe(message);
      }
    );
    ...
    `(
      ...
      async ({ language, message }) => {
        await addAdmin();
        const token = await getToken();
        const response = await putHidePost('does_not_exist', {
          token,
          language,
        });
        expect(response.body.message).toBe(message);
